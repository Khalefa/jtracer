...
public class JDI2JSON {
  ...
  public ArrayList<JsonObject> convertExecutionPoint(Event e, Location loc, ThreadReference t) {
    ...
    if (loc.method().name().indexOf("access$") >= 0)
      return results; // don't visualize synthetic access$000 methods
  ...
  public static String[] PU_stdlib = {"BinaryIn", "BinaryOut", "BinaryStdIn", "BinaryStdOut",
      "Copy", "Draw", "DrawListener", "In", "InTest", "Out", "Picture", "StdArrayIO", "StdAudio",
      "StdDraw", "StdDraw3D", "StdIn", "StdInTest", "StdOut", "StdRandom", "StdStats", "Stopwatch",
      "Stack", "Queue", "ST", "Point", "ST"};
        ...
        if (me.getKey().equals("event") || me.getKey().equals("line")) {
          ...
          result.add("stack_to_render",
              jsonModifiedArray(old_stack_to_render, 0,
                  jsonModifiedObject(
                      jsonModifiedObject(old_top_frame, "encoded_locals",
  ...
  private JsonObject stripFrameIDs(JsonObject ep) {
    ...
    for (JsonValue frame : (JsonArray) (ep.get("stack_to_render"))) {
      result.add(
          jsonModifiedObject(jsonModifiedObject((JsonObject) frame, "unique_hash", jsonString("")),
              "frame_id", jsonInt(0)));
    }
  ...
  private JsonObjectBuilder convertFrame(StackFrame sf, boolean highlight, JsonValue returnValue) {
    ...
    if (JDWPerror) {
      result.add("&hellip;?",
          jsonArray(
              "NUMBER-LITERAL", jsonString("&hellip;?"))); // hack since number-literal is just html
  ...
  private JsonValue convertObject(ObjectReference obj, boolean fullVersion) {
    ...
    // in the heap
      ...
      for (int i = 0; i < L; i++) {
        ...
        if (help.isz(ao.getValue(i))) {
          ...
          int j = i + 1;
          while (j < L && help.isz(ao.getValue(j))) j++;
          if (j - i >= 4) {
            ...
            for (int k = i; k < j; k++) result.add(convertValue(ao.getValue(k)));
    ...
    // stack and queue handling code by Will Gwozdz
      ...
      if (obj.referenceType().name().equals("ST")) {
          ...
          public void loadResultFromSymbolTree(ObjectReference n, JsonArrayBuilder result) {
            ...
            if (n.getValue(value) != null) {
              result.add(Json.createArrayBuilder()
                             .add(convertValue(n.getValue(key)))
                             .add(convertValue(n.getValue(value)))
                             .build());
            }
        ...
        new stHandler().loadResultFromSymbolTree(
            firstNode, result); // recursively add key, value pairs to the result
        return result.build();
      }
  ...
  private JsonValue convertValue(Value v) {
    ...
    // some longs can't be represented as doubles, they won't survive the json conversion
      ...
      return jsonArray("NUMBER-LITERAL", jsonString("" + ((LongValue) v).value()));
    ...
    // also, this lets us pass "Infinity" and other IEEE non-numbers
      ...
      return jsonArray("NUMBER-LITERAL", jsonString("" + ((FloatValue) v).value()));
    else if (v instanceof DoubleValue)
      return jsonArray("NUMBER-LITERAL", jsonString("" + ((DoubleValue) v).value()));
    else if (v instanceof CharValue)
      return jsonArray("CHAR-LITERAL", jsonString(((CharValue) v).value() + ""));
